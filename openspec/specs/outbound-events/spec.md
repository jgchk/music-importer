# outbound-events Specification

## Purpose

Publish the importer's adjudications as signed webhook events: the event store is the outbox, a checkpointed per-subscriber publisher delivers `release.verdict` at-least-once with Standard Webhooks signing, and the payload contract is producer-owned, additive-only, and frozen-fixture-verified.
